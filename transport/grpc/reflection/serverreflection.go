...
func Register(s *yarpc.Dispatcher) {
	ms := &serverReflectionServer{s: s}
	s.Register(rpb.BuildServerReflectionYARPCProcedures(ms))
}
	...
	s.initSymbols.Do(func() {
		...
		for _, p := range s.s.Router().Procedures() {
			...
			case transport.Unary:
				h, ok = p.HandlerSpec.Unary().(protobuf.ProtoHandler)
			case transport.Oneway:
				h, ok = p.HandlerSpec.Oneway().(protobuf.ProtoHandler)
			case transport.Streaming:
				h, ok = p.HandlerSpec.Stream().(protobuf.ProtoHandler)
			default:
...
func (s *serverReflectionServer) ServerReflectionInfo(stream rpb.ServerReflectionServiceServerReflectionInfoYARPCServer) error {
	for {
		in, err := stream.Recv()
		if err == io.EOF {
			return nil
