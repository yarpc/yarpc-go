...
-   **Breaking**: the HTTP inbound constructor now accepts only a `peer.Chooser`.
    For convenience, the common case of an outbound to a single peer
    may use a single peer chooser. Peer choosers require an agent to
    get the unique peer for a peer identifier.
