...
-   x/grpc: Remove `yarpc-grpc-app-` application header prefix.
-   x/grpc: Require `rpc-caller`, `rpc-service`, `rpc-encoding` headers.
    `rpc-encoding` may be substituted for `content-type` if `content-type`
    follows the content subtype structure for gRPC.
-   x/grpc: Add the `grpcheader` package to make it easier to deal with
    required x/grpc headers when using native gRPC libraries.
-   x/grpc: Start work on configuration handling.
-   x/protobuf: Change the `yarpc-protobuf-raw-response` header key to
    `rpc-protobuf-raw-response`.
-   x/protobuf: Change the name of the encoding from `protobuf` to `proto`.
-   x/protobuf: Handle JSON-encoded protobuf requests and return JSON-encoded
    protobuf responses if the `rpc-encoding` header is set to `json`, and allow
    protobuf clients to use JSON by using the `protobuf.UseJSON` option.
-   x/protobuf: Call `yarpc.RegisterClientBuilder` from code generated by
    `protoc-gen-yarpc-go`.
