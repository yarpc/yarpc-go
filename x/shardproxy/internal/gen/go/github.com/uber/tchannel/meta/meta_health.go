...
type Meta_Health_Args struct {
	Hr *HealthRequest `json:"hr,omitempty"`
}
...
func _HealthRequest_Read(w wire.Value) (*HealthRequest, error) {
	var v HealthRequest
	err := v.FromWire(w)
	return &v, err
}
	...
	for _, field := range w.GetStruct().Fields {
		...
		case 1:
			if field.Value.Type() == wire.TStruct {
				v.Hr, err = _HealthRequest_Read(field.Value)
				if err != nil {
					return err
				}
...
func (v *Meta_Health_Args) String() string {
	...
	i := 0
	if v.Hr != nil {
		fields[i] = fmt.Sprintf("Hr: %v", v.Hr)
		i++
	}
...
func (v *Meta_Health_Args) Equals(rhs *Meta_Health_Args) bool {
	if !((v.Hr == nil && rhs.Hr == nil) || (v.Hr != nil && rhs.Hr != nil && v.Hr.Equals(rhs.Hr))) {
		return false
	}
